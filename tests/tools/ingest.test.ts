...
beforeEach(() => {
  ...
  writeFileSync(join(TEST_DIR, "raw/article.md"), "# Great Article\n\nThis is about transformers.", "utf-8");
  writeFileSync(join(TEST_DIR, "wiki/existing-page.md"), "---\ntitle: Existing Page\ntype: concept\n---\n\nAlready here.", "utf-8");
});
...
describe("wiki_ingest handler", () => {
  ...
  it("throws if source does not exist", async () => {
    await expect(handleIngest({ source_path: "raw/nonexistent.md" }, TEST_DIR, DEFAULT_SCHEMA)).rejects.toThrow("not found");
  });
});
